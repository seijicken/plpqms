...
require 'PHPMailer/src/SMTP.php';
   ...
   if(isset($_POST['reguser'])){
         ...
         $user_password = password_hash($_POST['hr_password'], PASSWORD_DEFAULT, array('cost' => 12));  //PASSWORD_ARGON2I//PASSWORD_ARGON2ID
         $user_status = mysqli_real_escape_string($conn,$_POST['user_status']);
    

	$q_checkadmin = $conn->query("SELECT * FROM `hr_user` WHERE `hr_email` = '$email_address'") or die(mysqli_error($conn));
   ...
   timer:3000
    ...
    //Optional name
     ...
     $mail->send();
			...
			$conn->query("INSERT INTO `hr_user` VALUES('0','$user_name', '$email_address', '$user_password', '$user_status')") or die(mysqli_error($conn));
            echo "<script type='text/javascript'>Swal.fire({
  icon: 'success',
