...
if(isset($_POST["email"])){
  ...
  $email = $_POST['email'];
    ...
    $queryAdmin = "SELECT * FROM personnel_login WHERE email_address ='$email'";
                $resultAdmin =  mysqli_query($conn, $queryAdmin) or die ("Error in query: $queryAdmin. ".mysqli_error($conn)); 
                $rowAdmin = mysqli_fetch_assoc($resultAdmin);
                $name=$rowAdmin['name'];
                $email = $rowAdmin['email_address'];


    if($rowAdmin['email_address']>0){
...
<div class="form-floating mb-4 mt-0" >
      ...
      <label for="email" id="emailph" textContent="">Check Personnel Email</label>
    </div>
    <div class="ms-2">
