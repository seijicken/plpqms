                        ...
                        <center>
                          <div class="col-md-10 mt-4">
                            <div class="input-group">
                              <input class="form-control fw-bold rounded-3" type="file" name="pdfFile" id="pdfFile"
                                accept=".pdf" required>
                            </div>
                          </div>
                        </center>
                          ...
                          <div class="col-md-10 mt-2">
                            <div class="input-group">
                              ...
                              <datalist class="" aria-label="Default select example" id="emailemployee" required
                                ...
                                $query1 = "SELECT * FROM hr_user WHERE NAME!='$username'";
                                $result1 = mysqli_query($conn, $query1);
                                while ($file1 = mysqli_fetch_assoc($result1)) {

                                  $sendUser1 = $file1['hr_email'];
                        ...
                        <div class="row d-flex justify-content-center">
                          ...
                          <div class="col-md-5 mt-2">
                            <input type="text" maxlength="8" onkeypress="{return isNumberKey(event)}"
                              ...
                              placeholder="ID Control# 8 Digits Only" name="controlno" disabled>
...
if (isset($_POST['save'])) { // if the save button on the form is clicked
  ...
  for ($i = 0; $i < $countfiles; $i++) {
    ...
    $originalFilename = $_FILES['myfile']['name'][$i];
    $extension = pathinfo($originalFilename, PATHINFO_EXTENSION);
    $filename = 'PLP_' . $controlno . $originalFilename;
    ...
    if ($counter > 0) {
      // Generate a new filename with versioning
      $version = 1;
      $newFilename = $filename;

      while ($counter > 0) {
        $newFilename = pathinfo($filename, PATHINFO_FILENAME) . '(' . $version . ').' . $extension;
        $query = mysqli_query($conn, "SELECT NAME FROM `upload_files` WHERE `NAME` = '$newFilename'") or die(mysqli_error($conn));
        $counter = mysqli_num_rows($query);
        $version++;
      }
    ...
    if (!in_array($extension, ['pdf', 'docx', 'pptx', 'ppt', 'xlsx', 'xls', 'odt'])) {
    ...
    } else if ($_FILES['myfile']['size'][$i] > 52428800) {
      echo "<script type='text/javascript'>
                document.querySelector('body').style.overflow = 'hidden';
                Swal.fire({
                    icon: 'error',
      ...
      if (move_uploaded_file($file, $filePath)) {
        $sql = "INSERT INTO upload_files (controlNumber, NAME, DEPARTMENT, SIZE, DESCRIPTION, SENTBY, SENDTO, DOWNLOAD, TIMERS, TIMEDOWNLOAD, DESIGNATION, ADMIN_STATUS, EMAIL, YEAR, SEEN_STATUS, ARCHIVE, file_status, file_path) VALUES 
                ('$controlno', '$filename', '$foldername', '$size', '$description', '$user', '$sendto', 0, '$time', 0, '$designation', 'Admin', '$user', '$year', 0, 0, 'In Progress', '$filePath')";
