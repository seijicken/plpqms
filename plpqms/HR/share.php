...
<body>
  ...
  <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
    ...
    <symbol id="plane"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 1024 1024"
        height="2em" width="2em" xmlns="http://www.w3.org/2000/svg">
        <path
              ...
              <tbody>
                ...
                while ($file = mysqli_fetch_array($result)) {
                  ...
                  <tr>
                    ...
                    <td>
                      <a href='view_document.php?controlNumber=<?php echo $id; ?>' onclick="viewDocument(event)"
                      ...
                      <a href="share.php?id=<?php echo $id; ?>" class="btn btn-sm btn-outline-success"><i
                          class="fas fa-share" data-bs-toggle="modal" data-bs-target="#share"></i></a>
                      ...
                      <a href="archive.php?ID=<?php echo $id; ?>" class="btn btn-sm btn-outline-danger"
                        onclick="yesNO(event)"><i class="fa fa-trash"></i>
                        <script>
                          function yesNO(ev) {
                            document.querySelector("body").style.overflow = 'hidden';
                            ev.preventDefault();
      ...
      <form method="POST" action="shareProcess.php">
        <div class="modal-content rounded-5">
          <div class="modal-header text-center rounded-5">
            <h4 class="modal-title w-100 font-weight-bold"><svg class="" width="32" height="32">
                <use xlink:href="#plane" />
              </svg> SEND FILE TO USER</h4>
          ...
          <div class="modal-body">
            ...
            <div class="md-form mb-2">
              <input type="hidden" class="form-control" placeholder="Enter New Name" name="id"
                value="<?php echo $idname; ?>">
              <input type="hidden" class="form-control" placeholder="Enter New Name" name="controlNumber"
                value="<?php echo $controlno; ?>">
            </div>
            <div class="form-floating mb-3">
              <input type="text" id="orangeForm-name" name="filename" placeholder="Enter New Name"
                value="<?php echo $fname1; ?>" readonly class="form-control validate ms-0  rounded-4">
              <label for="email"><svg class="" width="20" height="20">
                  <use xlink:href="#email" />
                </svg> File Name</label>
            </div>
            <div class="form-floating mb-3">
              <textarea class="form-control  rounded-4" id="exampleFormControlTextarea1" name="description" rows="4"
                placeholder="Enter Description Message" style="height: 150px; resize: none;"></textarea>
              <label for="email"><svg class="" width="20" height="20">
                  <use xlink:href="#email" />
                </svg> Description Message</label>
            </div>
            <div class="form-floating mb-3">
              <input list="emailemployee" placeholder="Send to" name="sendTo" class="form-control rounded-3" required>

              <datalist id="emailemployee">
                ...
                // Fetching from login_user
                $query = "SELECT * FROM login_user WHERE NAME != '$username'";
                $result = mysqli_query($conn, $query);
                if ($result) {
                  while ($file = mysqli_fetch_assoc($result)) {
                    $sendUser = $file['email_address'];
                    echo "<option value='$sendUser'>Send to User Acc: $sendUser</option>";
                  }
                } else {
                  echo "<option value=''>Error loading users</option>";
                }

                // Fetching from hr_user
                $query2 = "SELECT * FROM hr_user WHERE NAME != '$username'";
                $result2 = mysqli_query($conn, $query2);
                if ($result2) {
                  while ($file2 = mysqli_fetch_assoc($result2)) {
                    $sendUser2 = $file2['hr_email']; // Ensure the field name is correct
                    echo "<option value='$sendUser2'>Send to HR User Acc: $sendUser2</option>";
                  }
                } else {
                  echo "<option value=''>Error loading HR users</option>";
                }

                // Fetching from admin_login
                $query3 = "SELECT * FROM admin_login WHERE NAME != '$username'";
                $result3 = mysqli_query($conn, $query3);
                if ($result3) {
                  while ($file3 = mysqli_fetch_assoc($result3)) {
                    $sendUser3 = $file3['admin_user']; // Ensure the field name is correct
                    echo "<option value='$sendUser3'>Send to Admin Acc: $sendUser3</option>";
                  }
                } else {
                  echo "<option value=''>Error loading Admin users</option>";
                }
      ...
      <div class="modal-dialog modal-dialog-centered border-bottom border-top border-5 rounded-4" role="document">
        <div class="modal-content bg-light">
          ...
          <div class="modal-body">
            ...
            <div class="form mb-3">
              <label class="ms-1 mb-1"><svg class="" width="20" height="20">
            ...
            <div class="form-floating mb-3">
              <input type="password" class="form-control" id="pass" placeholder="Enter password" name="admin_password"
                required>
              <label for="email"><svg class="" width="20" height="20">
                  <use xlink:href="#lock" />
