...
<div class="row d-md-block w-100 ms-0">
    ...
    <ul class="nav nav-pills flex-column mb-auto px-1"id="itemSidebar">
      ...
      <a href="manage_folder.php" class="nav-link text-white dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
          <svg class="bi pe-none me-2" width="16" height="16"><use xlink:href="#folder"/></svg>
          Manage Folders
        </a><ul class="dropdown-menu fw-normal border-top border-bottom border-4 border-warning ms-3" style="background-color: green;"><li>
    ...
    <li>
      ...
      <li>
        ...
        <a href="view_hr.php" class="nav-link dropdown-item text-white" style="background-color: rgba(202, 202, 202, .65);">
          <svg class="bi pe-none me-2" width="16" height="16"><use xlink:href="#folder"/></svg>
           - Admins Accounts
        </a></li>
        <li>
   ...
   <div class="container-fluid mt-5">
      ...
      <div class="card mb-4 wow fadeIn">
        ...
        <h4 class="mb-2 mb-sm-0 pt-1 fw-bold">
            <a href="home.php" style="text-decoration: none; color:green; ">HOME PAGE | VIEW ACCOUNT</a>
            <span>|</span>
          ...
          ?>       
           ...
           <tr>
              <td width='5%' class="fw-bold"><?php echo $idname; ?></td>
               ...
               <td width='8%'><a href="edit_hr.php?id=<?php echo $rs['id'];?>"><svg class="" href="tryEdit.php" width="20" height="20"><use xlink:href="#edit"/></svg></a> | <a href="delete_admin.php?id=<?php echo htmlentities($rs['id']); ?>" onclick="yesNO(event)"><svg class="" width="20" height="20" style="color:red; cursor: hand;"><use xlink:href="#trash"/></svg>
 <script>
              function yesNO(ev){
                ev.preventDefault();
...
<script type="text/javascript" src="https://cdn.datatables.net/v/dt/jq-3.6.0/dt-1.12.1/r-2.3.0/rr-1.2.8/datatables.min.js"></script>
  ...
  <div class="modal-dialog modal-dialog-centered border-bottom border-top border-5 rounded-4" role="document">
    ...
    <div class="form-floating mb-3">
        <input type="text" class="form-control" id="email" placeholder="Enter email" name="hr_email">
        <label for="email"><svg class="" width="20" height="20"><use xlink:href="#email"/></svg>   Enter Email</label>
    </div>

      <div class="form-floating mb-3">
        <input type="password" class="form-control" id="pass" placeholder="Enter password" name="hr_password" required>
...
if (isset($_POST['edit'])) {
    ...
    $email_address = mysqli_real_escape_string($conn, $_POST['hr_email']);

    // Check if the email already exists in the database
    $q_checkemail1 = $conn->query("SELECT * FROM `hr_user` WHERE `hr_email` = '$email_address' AND `id` != {$_REQUEST['id']}") or die(mysqli_error());
    $v_checkemail1 = $q_checkemail1->num_rows;

    if ($v_checkemail1 > 0) {
        // Email already exists, fail the update
        echo "<script type='text/javascript'>
            document.querySelector('body').style.overflow = 'hidden';
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'The specified email address is already in use. Please choose a different email.',
    ...
    if ($v_checkemail1 == 0) {
        // Email doesn't exist, update with a new hashed password
        $user_password1 = password_hash($_POST['hr_password'], PASSWORD_DEFAULT, array('cost' => 12));
        $stmt = $conn->prepare("UPDATE `hr_user` SET `name` = ?, `hr_email` = ?, `hr_password` = ? WHERE `id` = ?");
        $stmt->bind_param("sssi", $user_name, $email_address, $user_password1, $_REQUEST['id']);
    } else {
        // Email exists, update without changing the password
        $stmt = $conn->prepare("UPDATE `hr_user` SET `name` = ?, `hr_email` = ? WHERE `id` = ?");
        $stmt->bind_param("ssi", $user_name, $email_address, $_REQUEST['id']);
    }
