<?php
error_log("Executing create_personnel.php");
...
if(isset($_POST)){
    ...
    $q_checkadmin = $conn->query("SELECT * FROM `personnel_login` WHERE `email_address` = '$email_address'") or die(mysqli_error($conn));
    ...
    if($v_checkadmin == 1){
        ...
        $status = 'Faculty';
        $query = mysqli_query($conn, "INSERT INTO newlycreatedpersonnel(email, status) VALUES ('$email_address', '$status')");
        if(!$query){
            exit("Error");
        }
        ...
        try {
            ...
            $conn->query("INSERT INTO `personnel_login` VALUES('0','$user_name', '$email_address', '$user_password', '$user_status', '$designation')") or die(mysqli_error($conn));
            echo "<script type='text/javascript'>Swal.fire({
                icon: 'success',
                title: 'Succesfully Created!!',
                text:'Faculty Account is Successfully Created, The Login Credentials is Sent to the Inputted Email.',
                confirmButtonColor:'#00396D',
                confirmButtonText: 'Okay',
                timer:3000
            }).then(function(){
