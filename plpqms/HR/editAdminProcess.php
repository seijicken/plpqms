...
if (isset($_POST['edit'])) {
    ...
    $q_checkemail1 = $conn->query("SELECT * FROM `admin_login` WHERE `admin_user` = '$email_address' AND `id` != {$_REQUEST['id']}") or die(mysqli_error());
    ...
    if ($v_checkemail1 == 0) {
        ...
        $stmt = $conn->prepare("UPDATE `admin_login` SET `name` = ?, `admin_user` = ?, `admin_password` = ? WHERE `id` = ?");
        $stmt->bind_param("sssi", $user_name, $email_address, $user_password1, $_REQUEST['id']);
    } else {
        // Email exists, update without changing the password
        $stmt = $conn->prepare("UPDATE `admin_login` SET `name` = ?, `admin_user` = ? WHERE `id` = ?");
