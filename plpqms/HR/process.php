...
<div class="col-md-5 mt-2">
        <input type="text" maxlength="8" onkeypress="{return isNumberKey(event)}" inputmode="numeric" class="form-control rounded-3" id="email" placeholder="ID Control# 8 Digits Only" name="controlno" disabled>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES["pdfFile"]) && $_FILES["pdfFile"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES["pdfFile"]["tmp_name"], $uploadFile)) {
            ...
            $pythonScriptPath = "C:\\xampp\\htdocs\\BJEN\\plpqmsV1.12\\plpqms\\4plp\\signature_extractor_and_pdf_to_img\\sample_project\\main.py";
            ...
            if (strpos($output, 'Genuine') !== false) {
                ...
                $sql = "INSERT INTO upload_files (controlNumber, NAME, DEPARTMENT, SIZE, DESCRIPTION, SENTBY, SENDTO, DOWNLOAD, TIMERS, TIMEDOWNLOAD, DESIGNATION, ADMIN_STATUS, EMAIL, YEAR, SEEN_STATUS, ARCHIVE) 
                        VALUES ('$controlno','$filename','$foldername', '{$_FILES['pdfFile']['size']}', '$description','$user', '$sendto', 0, '$time', 0, '$designation', 'Admin', '$user', '$year', 0, 0)";
                ...
                // If forged, set 'send to' value to the user itself
                $sendto = $user;
                ...
                echo "<script type='text/javascript'>
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Failed Uploading the File!!',
                            text: 'Your File Does Not Have Signature!!',
                            confirmButtonColor: '#00396D',
                            confirmButtonText: 'Okay',
                            timer: 3000
                        }).then(function(){
            ...
            // Redirect back to the previous page with the alert message as a query parameter
            header("Location: {$_SERVER['HTTP_REFERER']}?alertMessage=" . urlencode($alertMessage));
            exit();
        } else {
