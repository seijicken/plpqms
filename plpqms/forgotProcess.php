...
<div id="carouselExampleIndicators" class="col-6 carousel slide" data-bs-ride="true" style="margin-top:-171px;">
  ...
  <div class="container login-sec">
    <img src="../assets/img/rtudms1-forgot.png" class="img-fluid mt-3" title="e-PLP" alt="e-PLP">
    <hr class="mb-2">
...
if(isset($_POST["email"])){
  ...
  $emailTo = $_POST["email"];
  $emailcompare = $_POST['email'];

    $queryAdmin = "SELECT * FROM admin_login WHERE admin_user ='$emailcompare'";
                $resultAdmin =  mysqli_query($conn, $queryAdmin) or die ("Error in query: $queryAdmin. ".mysqli_error($conn)); 
                $rowAdmin = mysqli_fetch_assoc($resultAdmin);
                $name=$rowAdmin['name'];
                $emaildata = $rowAdmin['admin_user'];
   ...
   if(!empty($emaildata)){
    ...
    $mail->Username   = 'rosario_greta@plpasig.edu.ph';                     //SMTP username
    $mail->Password   = 'ajwxvzadmzgzgypk';                         //SMTP password
